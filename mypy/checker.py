...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def visit_func_def(self, defn: FuncDef) -> None:
        ...
        if (
            ...
            and (
                ...
                or (
                    defn.unanalyzed_type
                    and isinstance(defn.unanalyzed_type, CallableType)
                    and (
                        is_unannotated_any(defn.unanalyzed_type.ret_type)
                        or any(
                            is_unannotated_any(typ) for typ in defn.unanalyzed_type.arg_types[1:]
                        )
                    )
                )
            )
            ...
            for base in defn.info.mro[1:]:
                ...
                super_types.update(
                    {
                        arg: UnionType.make_union(arg_type)
                        for arg, arg_type in item_arg_types.items()
                    }
                )
                any_ = UntypedType()
    ...
    def check_func_def(self, defn: FuncItem, typ: CallableType, name: Optional[str]) -> None:
        ...
        if (
            defn.info
            and self.options.infer_function_types
            and not (defn.type and isinstance(defn.type, CallableType) and defn.type.fully_typed)
        ):
            for base in defn.info.mro[1:]:
                super_ = base.names.get(defn.name)
                ...
                for arg_i, arg_name in enumerate(defn.arg_names):
                    ...
                    if arg_i == 0 and not defn.is_static:
                        ...
                        continue
                    if isinstance(defn.type, CallableType) and not isinstance(
                        get_proper_type(defn.type.arg_types[arg_i]), AnyType
                    ):
                        continue
    ...
    def fixup_partial_type(self, typ: Type) -> Type:
        ...
        if typ.type is None:
            ...
            return Instance(typ.type, [UntypedType()] * len(typ.type.type_vars))
